...
export default function DeliveryPage() {
  ...
  async function loadDeliveries() {
    ...
    try {
      ...
      const response = await client.get<Delivery[]>('/deliveries', { params });
  ...
  function addLine() {
    setForm((value) => ({
      ...value,
      lines: [...value.lines, { product_id: '', demand_qty: 1, location_id: '' }],
    }));
  }
  ...
  async function createDelivery(event: React.FormEvent) {
    ...
    const validLines = form.lines.filter((line) => line.product_id && line.demand_qty > 0);
  ...
  async function setStatus(delivery: Delivery, status: string) {
    try {
      await client.put(`/deliveries/${delivery.id}`, {
      ...
      toast.success(`Delivery moved to ${status}`);
      await loadDeliveries();
    } catch (error) {
  ...
  return (
    <div className="page-container">
      <PageHeader
        ...
        subtitle="Track outbound shipments and decrement stock on validation."
        actions={(
              ...
              {loading ? (
                ...
                deliveries.map((delivery) => (
                  <tr key={delivery.id}>
                    ...
                    <td>{formatQty(delivery.lines.length)}</td>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                        {delivery.status !== 'done' ? (
                          <select
                            ...
                            onChange={(event) => setStatus(delivery, event.target.value)}
                          >
                            {operationStatusOptions.map((status) => (
                        ...
                        <Link className="btn btn-secondary btn-sm" to={`/deliveries/${delivery.id}`}>
                          Open
                        </Link>
                      </div>
                    </td>
        ...
        {drawerOpen ? (
          <>
            ...
            >
              <div className="drawer-header">
                <div>
                  ...
                  <p style={{ color: 'var(--txt-muted)', fontSize: '0.8rem' }}>
                    Add customer, warehouse and outgoing lines.
                  </p>
                </div>
              ...
              <form className="drawer-body" onSubmit={createDelivery} style={{ display: 'grid', gap: 12 }}>
